...
# 장애 처리 앱 - macOS x86 앱 번들 생성 스크립트

set -e  # 오류 발생 시 즉시 중단

echo "🚀 장애 처리 앱 번들 생성 시작 (x86_64 아키텍처)..."
...
APP_NAME="IncidentApp-x86"
APP_DISPLAY_NAME="장애 처리 (x86)"
BUNDLE_ID="com.gsretail.incidentapp.x86"
VERSION="1.0.0"
BUILD_NUMBER="1"
...
echo -e "${BLUE}ℹ️  x86_64 아키텍처로 빌드합니다${NC}"
...
# x86_64 아키텍처로 릴리즈 빌드
echo -e "${YELLOW}🔨 x86_64 릴리즈 빌드 진행 중...${NC}"
...
echo -e "${GREEN}✅ x86_64 빌드 성공${NC}"

# 앱 번들 구조 생성
echo -e "${YELLOW}📦 x86 앱 번들 생성 중...${NC}"
...
# 실행 파일 복사 (원본 이름 사용)
echo "  실행 파일 복사..."
cp ".build/$SWIFT_ARCH/release/IncidentApp" "$MACOS_DIR/$APP_NAME"
...
if [ -f "$MACOS_DIR/$APP_NAME" ]; then
    ...
    # 아키텍처 확인
    echo -e "${YELLOW}🔍 아키텍처 확인...${NC}"
    file_arch=$(lipo -info "$MACOS_DIR/$APP_NAME" 2>/dev/null | grep -o 'x86_64' || echo "")
    if [ "$file_arch" = "x86_64" ]; then
        echo -e "${GREEN}✅ x86_64 아키텍처 확인됨${NC}"
    else
        echo -e "${YELLOW}⚠️  아키텍처 확인: $(lipo -info "$MACOS_DIR/$APP_NAME" 2>/dev/null || echo "확인 불가")${NC}"
    fi
else
...
# 완료 메시지
echo ""
echo -e "${GREEN}🎉 x86 앱 번들 생성 완료!${NC}"
echo -e "${BLUE}📍 앱 위치: $APP_BUNDLE${NC}"
echo -e "${BLUE}ℹ️  아키텍처: x86_64 (Intel Mac)${NC}"
echo ""
echo "사용 방법:"
